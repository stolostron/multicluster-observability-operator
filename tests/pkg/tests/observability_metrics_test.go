...
var _ = Describe("", func() {
	...
	It("RHACM4K-1449 - Observability - Verify metrics data consistency [P2][Sev2][Observability][Integration]@ocpInterop @e2e @post-release (metrics/g1)", func() {
	...
	It("RHACM4K-1658: Observability: Customized metrics data are collected [P2][Sev2][Observability][Integration]@ocpInterop @e2e @post-release (metrics/g0)", func() {
	...
	It("RHACM4K-3063: Observability: Metrics removal from default allowlist [P2][Sev2][Observability][Integration]@ocpInterop @e2e @post-release (metrics/g0)", func() {
	...
	It("RHACM4K-3339: Observability: Verify recording rule - Should have metrics which used grafana dashboard [P2][Sev2][Observability][Integration]@ocpInterop @e2e @post-release (ssli/g1)", func() {
		metricList := utils.GetDefaultMetricList(testOptions)
		ignoreMetricMap := utils.GetIgnoreMetricMap()
		_, etcdPodList := utils.GetPodList(
			testOptions,
