...
var _ = Describe("", func() {
	...
	It("RHACM4K-31474: Observability: Verify memcached setting max_item_size is populated on thanos-store - [P1][Sev1][Observability][Stable]@ocpInterop @post-upgrade @e2e @post-release(config/g1)", func() {
	...
	It("RHACM4K-31475: Observability: Verify memcached setting max_item_size is populated on thanos-query-frontend - [P1][Sev1][Observability][Stable]@ocpInterop @post-upgrade @e2e @post-release(config/g1)", func() {
	...
	It("RHACM4K-1235: Observability: Verify metrics data global setting on the managed cluster @BVT - [P1][Sev1][Observability][Stable]@ocpInterop @post-upgrade @e2e @post-release(config/g0)", func() {
		/*
	...
	It("RHACM4K-1065: Observability: Verify MCO CR storage class and PVC @BVT - [P1][Sev1][Observability][Stable]@ocpInterop @post-upgrade @e2e @post-release (config/g0)", func() {
		/*
			if os.Getenv("SKIP_INSTALL_STEP") == "true" {
				Skip("Skip the case due to MCO CR was created customized")
			}
	...
	It("RHACM4K-11169: Observability: Verify ACM Observability with Security Service Token credentials - [P2][Sev2][observability][Integration]@ocpInterop @post-upgrade @e2e @pre-upgrade Checking service account annotations is set for store/query/rule/compact/receive @e2e (config/g0)", func() {
