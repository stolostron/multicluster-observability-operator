...
func TestGeneratePrometheusRule_AllProfilesGenerated(t *testing.T) {
	...
	assert.Equal(t, "acm-vm-right-sizing-namespace-1d.rules", namespace1d.Name)
	assert.Len(t, namespace1d.Rules, 6*len(rsutility.RecommendationProfiles))

	cluster1d := rule.Spec.Groups[3]
	assert.Equal(t, "acm-vm-right-sizing-cluster-1d.rule", cluster1d.Name)
	assert.Len(t, cluster1d.Rules, 6*len(rsutility.RecommendationProfiles))
	...
	for _, p := range rsutility.RecommendationProfiles {
		assert.True(t, profileSeen[p.Name], "missing profile %s in virt namespace 1d rules", p.Name)
	}
