...
func TestCleanupRSVirtualizationResources_UsesCorrectComponentConfig(t *testing.T) {
	...
	assert.NoError(t, CleanupRSVirtualizationResources(ctx, client, rsutility.DefaultNamespace, false))
	assert.NoError(t, CleanupRSVirtualizationResources(ctx, client, rsutility.DefaultNamespace, true))
}
