...
func newAPISpec(c client.Client, mco *mcov1beta2.MultiClusterObservability) (obsv1alpha1.APISpec, error) {
	...
	apiSpec.Replicas = mcoconfig.GetReplicas(mcoconfig.ObservatoriumAPI, mco.Spec.WriteTShirtSize, mco.Spec.AdvancedConfig)
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		apiSpec.Resources = mcoconfig.GetResources(mcoconfig.ObservatoriumAPI, mco.Spec.WriteTShirtSize, mco.Spec.AdvancedConfig)
	}
...
func newReceiversSpec(
	...
	receSpec.Replicas = mcoconfig.GetReplicas(mcoconfig.ThanosReceive, mco.Spec.WriteTShirtSize, mco.Spec.AdvancedConfig)
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		receSpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosReceive, mco.Spec.WriteTShirtSize, mco.Spec.AdvancedConfig)
	}
...
func newRuleSpec(mco *mcov1beta2.MultiClusterObservability, scSelected string) obsv1alpha1.RuleSpec {
	...
	ruleSpec.Replicas = mcoconfig.GetReplicas(mcoconfig.ThanosRule, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)

	ruleSpec.ServiceMonitor = true
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		ruleSpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosRule, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
		if mco.Spec.ReadTShirtSize == "" {
			mco.Spec.ReadTShirtSize = mcoconfig.Small
		}
		ruleSpec.ReloaderResources = v1.ResourceRequirements{
			Requests: v1.ResourceList{
				v1.ResourceName(v1.ResourceCPU):    resource.MustParse(mcoconfig.ThanosRuleReloaderCPURequest[mco.Spec.ReadTShirtSize]),
				v1.ResourceName(v1.ResourceMemory): resource.MustParse(mcoconfig.ThanosRuleReloaderMemoryRequest[mco.Spec.ReadTShirtSize]),
			},
		}
	}
...
func newStoreSpec(mco *mcov1beta2.MultiClusterObservability, scSelected string) obsv1alpha1.StoreSpec {
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		storeSpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosStoreShard, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	}
	...
	storeSpec.Shards = mcoconfig.GetReplicas(mcoconfig.ThanosStoreShard, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
...
func newMemCacheSpec(component string, mco *mcov1beta2.MultiClusterObservability) obsv1alpha1.MemCacheSpec {
	...
	memCacheSpec.Replicas = mcoconfig.GetReplicas(component, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		memCacheSpec.Resources = mcoconfig.GetResources(component, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
		memCacheSpec.ExporterResources = mcoconfig.GetResources(mcoconfig.MemcachedExporter, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	}
...
func newQueryFrontendSpec(mco *mcov1beta2.MultiClusterObservability) obsv1alpha1.QueryFrontendSpec {
	...
	queryFrontendSpec.Replicas = mcoconfig.GetReplicas(mcoconfig.ThanosQueryFrontend, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	queryFrontendSpec.ServiceMonitor = true
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		queryFrontendSpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosQueryFrontend, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	}
...
func newQuerySpec(mco *mcov1beta2.MultiClusterObservability) obsv1alpha1.QuerySpec {
	...
	querySpec.Replicas = mcoconfig.GetReplicas(mcoconfig.ThanosQuery, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		querySpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosQuery, mco.Spec.ReadTShirtSize, mco.Spec.AdvancedConfig)
	}
...
func newReceiverControllerSpec(mco *mcov1beta2.MultiClusterObservability) obsv1alpha1.ReceiveControllerSpec {
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		if mco.Spec.WriteTShirtSize == "" {
			mco.Spec.WriteTShirtSize = mcoconfig.Small
		}
		receiveControllerSpec.Resources = v1.ResourceRequirements{
			Requests: v1.ResourceList{
				v1.ResourceName(v1.ResourceCPU): resource.MustParse(
					mcoconfig.ObservatoriumReceiveControllerCPURequest[mco.Spec.WriteTShirtSize],
				),
				v1.ResourceName(v1.ResourceMemory): resource.MustParse(
					mcoconfig.ObservatoriumReceiveControllerMemoryRequest[mco.Spec.WriteTShirtSize],
				),
			},
		}
	}
...
func newCompactSpec(mco *mcov1beta2.MultiClusterObservability, scSelected string) obsv1alpha1.CompactSpec {
	...
	if !mcoconfig.WithoutResourcesRequests(mco.GetAnnotations()) {
		compactSpec.Resources = mcoconfig.GetResources(mcoconfig.ThanosCompact, mco.Spec.WriteTShirtSize, mco.Spec.AdvancedConfig)
	}
