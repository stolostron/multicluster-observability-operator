...
update_mco_cr() {
  ...
  if [[ -n ${MULTICLUSTER_OBSERVABILITY_ADDON_IMAGE_REF} ]]; then
    ${SED_COMMAND} "/annotations.*/a \ \ \ \ mco-multicluster_observability_rhel9_addon-image: ${MULTICLUSTER_OBSERVABILITY_ADDON_IMAGE_REF}" ${ROOTDIR}/examples/mco/e2e/v1beta2/observability.yaml
    ${SED_COMMAND} "/annotations.*/a \ \ \ \ mco-multicluster_observability_rhel9_addon-image: ${MULTICLUSTER_OBSERVABILITY_ADDON_IMAGE_REF}" ${ROOTDIR}/examples/mco/e2e/v1beta2/*/observability.yaml
  fi
